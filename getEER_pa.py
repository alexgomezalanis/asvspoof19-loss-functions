...
def getLDAScores(X, Y):
  ...
  CM_SCOREFILE = rootWrite + '/PA_LCNN_lda.txt'
...
def getSVMScores(X, Y_binary):
  CM_SCOREFILE = rootWrite + '/PA_LCNN_svm.txt'
...
def getSVMOneScores(X_genuine, X_spoof):
  CM_SCOREFILE = rootWrite + '/PA_LCNN_svm_one.txt'
...
def getGMMScores(X_genuine, X_spoof):
  CM_SCOREFILE = rootWrite + '/PA_LCNN_gmm.txt'
  text_file = open(CM_SCOREFILE, "w")
